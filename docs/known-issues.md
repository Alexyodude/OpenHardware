...
| 3.2 | `.claude/rules/determinism.md` describes three mechanisms in prose but declares two in frontmatter. The original triage assumed the other rules declare their CONVENTION entries; the final review checked, and **four of five do not** — `core-interface.md` is the outlier. | Drop `core-interface.md`'s no-op CONVENTION entry rather than adding three more, then state the convention explicitly. |
