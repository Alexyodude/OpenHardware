...
| 1.4 | `tools/check_deltas.py` | Compares `fork-point..HEAD`, not the working tree. Correct for a CI gate, but a locally modified upstream file passes until committed. Not stated in `rules/upstream-sync.md`. | Document the choice in the rule. |
...
| 1.8 | `tools/check_deltas.py` | **Its premise — that everything in `fork-point..HEAD` is ours — is false for any tree also containing upstream commits made after `fork-point`.** Found the first time CI ever ran (2026-08-12): the PR checkout merges base into head, base carried upstream's two post-fork commits, and the checker reported their eight files as our unlogged modifications. Worked around in `rules.yml` by checking out the PR head instead of the merge commit, which is correct for the rule's subject but does not address the general case: **the day this fork merges `upstream/master` for real, the same eight files come back.** | Diff against the merge-base with upstream rather than the fixed tag, so upstream's own edits are never attributed to us. That is a semantic change to `rules/upstream-sync.md` §1 and needs the rule rewritten with it, not a quiet patch to the tool. |
...
| 4a.5 | **Withdrawn 2026-08-12 — this defect does not exist.** It said `get part[0].in[N]` returns ERROR for every N, so `Part->GetInputCount()` is 0 for a headlessly placed part. Measured again with a zero-padded index, `get part[00].in[00]` returns `part[00].in[00] PB_1= 0`. Input enumeration works; the original measurement used a one-digit index, which the fixed-width parser in 4a.8 mangles into an out-of-range value. | `webui.ui.button-press` and `webui.ui.pot-drag` are **not** blocked on enumeration. What is still unproven is the meaning of a *written* value: `set part[00].in[00] = 1` and `= 0` are both accepted and both read back 16. Reading works, writing needs its encoding pinned down. |
...
| 4a.4 | **Withdrawn 2026-08-12 — this defect does not exist.** It said `get board.in[]`/`get board.out[]` return ERROR on Arduino Uno because the board has no on-board controls. The Uno does have one: `info` advertises `board.out[01] LD_L`, the on-board L LED, and `get board.out[01]` returns `LD_L= 97`. The original measurement used `get board.out[1]`, which 4a.8 explains. | Board I/O is still board-dependent — the Uno reports one output and no inputs — but it is **not** absent, and a UI may render it. This is what makes `webui.ui.led-render` reachable on the default board. |
| 4a.8 | **Every indexed accessor in `rcontrol` is fixed-width two-digit, and a one-digit index fails as ERROR rather than as a parse error.** `src/lib/rcontrol.cc:748` computes `(ptr[11]-'0')*10 + (ptr[12]-'0')`, so `board.out[1]` reads `']'` as the second digit, yields 55, fails the range test, and returns ERROR. The same pattern indexes `board.in[`, `apin[`, and `part[N].in[M]`. | **This one parsing rule generated 4a.4 and 4a.5, two upstream defects that were never real.** ERROR is a single undifferentiated reply, so "malformed request" and "unsupported feature" cannot be told apart without reading the parser. Index formatting belongs in exactly one place in `webui/api.py`. Note the ceiling too: indices above 99 are unaddressable, which bounds parts at 100 for any indexed operation. |

| 4a.6 | **CORRECTED 2026-08-24. The original claim was that the NOGUI build cannot be linked with GCC 11.4 on Ubuntu 22.04. That is false, and upstream's own CI disproves it.** What is true: on *this* WSL box, `make -f Makefile.NOGUI` compiles cleanly, reaches the link stage, and dies with `lto1: internal compiler error: Segmentation fault`. Removing `-flto=auto` does not help, because the dependency archives carry LTO IR themselves (picsim and lxrad_nogui both hardcode `-flto` in their own makefiles). **But `.github/workflows/linux-release.yml` in the reference checkout runs a matrix of `[ubuntu-22.04, ubuntu-24.04]`, gates the appimage step to `ubuntu-22.04`, and that step runs `bscripts/build_appimage.sh` whose line 68 is the NOGUI link — successfully, on every master push.** The published artefact `PICSimLab_NOGUI-0.9.3_260822_Ubuntu_22.04.5_LTS_x86_64.AppImage` (16,435,704 bytes, HTTP 200, verified) was built from commit `62e8b5b`, which is the exact commit `../picsimlab-reference` sits on. So the ICE is **environmental to this machine**, not a property of (this source x GCC 11.4). Most likely causes, none confirmed: mixed-vintage LTO IR in a stale `build_all/`, WSL memory pressure during parallel LTRANS, or ccache staleness. Note also that the WX GUI build links the same LTO-carrying `libpicsim.a`/`libsimavr.a` here and works — so the NOGUI-only delta is `liblxrad_NOGUI.a`, which narrows the suspect considerably. `build_package_NOGUI.sh` fails separately on Debian packaging plumbing. | **Nothing here needs to be built at all.** Upstream publishes prebuilt NOGUI AppImages *and* Windows binaries, and the AppImage bundles `lib/qemu` — `libqemu-riscv32`, `libqemu-xtensa` and the ESP ROM images — which is what the ESP evaluation needs. `install_root()` requires only `share/`, so a binary install satisfies it. Try a clean `rm -rf build_all && ccache -C` rebuild before assuming a compiler problem, and if it recurs, raise the WSL memory cap. |
